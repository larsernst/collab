...
- [x] Add security headers, TLS deployment guidance, and secret-rotation procedures.
- [x] Add dependency and container vulnerability scanning.
- [x] Add multi-architecture container builds.
- [x] Add GitHub Actions image publishing and release versioning.
...
| 2026-06-19 | Phase 7 security headers, TLS, and secret rotation | Added server-wide and gateway-level browser hardening headers (`CSP`, `nosniff`, `Referrer-Policy`, `Permissions-Policy`, `COOP`, and legacy frame denial), plus a Caddy TLS example with HSTS for real HTTPS deployments. Added a TLS/secret-rotation runbook covering gateway topology, self-signed/private-server behavior, session/token invalidation, admin password rotation, PostgreSQL password rotation, TLS key rotation, and external backup credential rotation. Added `pnpm server:sessions:revoke` / `scripts/server-revoke-sessions.sh` to invalidate every active browser and native session after a suspected leak or credential rotation. | `cargo fmt --all`; `cargo test -p collab-server app::tests::responses_include_security_headers`; `cargo check -p collab-server`; `bash -n scripts/server-revoke-sessions.sh`; `pnpm exec tsc --noEmit`; `docker compose config`; `git diff --check` | Next Phase 7 task: dependency and container vulnerability scanning |
| 2026-06-19 | Phase 7 dependency and container vulnerability scanning | Added a local vulnerability-scan command (`pnpm server:security:scan`) that runs `pnpm audit`, `cargo audit`, and a server-image scan via Trivy/Grype/Docker Scout. Added a GitHub `Security Scan` workflow that runs on every push and by manual dispatch, but not on pull-request events; it fails high/critical dependency findings and scans the built `Dockerfile.server` image with Trivy while uploading SARIF to GitHub code scanning. Documented local scanner setup, CI gate behavior, finding triage, and exception rules. | `bash -n scripts/server-security-scan.sh`; `pnpm exec tsc --noEmit`; `docker compose config`; workflow and scanner docs review; `git diff --check` | Next Phase 7 task: multi-architecture container builds |
| 2026-06-19 | Phase 7 multi-architecture container builds | Added `pnpm server:image:build` / `scripts/server-container-build.sh` to build a combined `linux/amd64` + `linux/arm64` OCI image index locally with Buildx, while preserving environment overrides and an explicit push mode for future release automation. Added a manually dispatchable `Server Container Build` CI workflow with QEMU, architecture-scoped BuildKit caches, and separate short-lived AMD64/ARM64 OCI artifacts; automatic runs are reserved for version tags and the release publishing path. Documented local cross-build requirements, artifact behavior, and release automation. | `bash -n scripts/server-container-build.sh`; helper CLI/error-path tests; workflow YAML parse; `docker buildx version`; `docker compose config`; `git diff --check` | Next Phase 7 task: GitHub Actions image publishing and release versioning |
| 2026-06-19 | Phase 7 image publishing and release versioning | Extended the server-container workflow to publish release-tag builds to GHCR as one AMD64/ARM64 manifest. A release gate (`pnpm server:release:check vX.Y.Z`) requires semantic tags to match the root package, admin package, Cargo workspace, and Tauri package versions; builds and blocks on a high/critical Trivy scan before registry login/push. Published images receive exact-version, moving-minor, and stable `latest` tags plus OCI metadata, maximal BuildKit provenance, and an SBOM. Compose now accepts `COLLAB_SERVER_IMAGE` so operators can deploy a pinned tag/digest with `--no-build`, while local source builds retain `collab-server:local`. | release-version checker success/mismatch/malformed tests; workflow YAML parse; `docker compose config`; `git diff --check` | Next Phase 7 task: document supported deployment topology and upgrade compatibility |
| 2026-06-20 | Phase 7 security-scan follow-up | Cleared the high-severity JavaScript audit findings by updating Vite, React Router, jsdom, and the shadcn CLI dependency graph, while moving shadcn to development scope. Regenerated the lockfile without unsafe transitive overrides; the full audit now selects patched Undici, path-to-regexp, fast-uri, and Hono releases and remains the blocking gate. Replaced the RustSec and Trivy wrapper actions with direct pinned CLI execution to avoid removed nested actions and unavailable Checks API permissions. Updated `rustls-webpki` for three certificate/CRL advisories; the only ignored vulnerability is the unfixed `rsa` timing advisory reachable solely through SQLx's unused compile-time MySQL backend. The container gate checksum-verifies Trivy `0.71.2`, scans a source-built Docker archive directly, and prints findings in the job log without requiring GitHub code scanning. | `pnpm audit --audit-level high` (0 high/critical; 6 low, 16 moderate); `cargo audit` (0 unignored vulnerabilities; 22 visible warnings); checksum-verified Trivy `0.71.2` install and CLI flag check; `pnpm install --frozen-lockfile`; `pnpm test` (517); `pnpm admin:test` (39); `pnpm exec tsc --noEmit`; `pnpm admin:build`; `cargo check --workspace`; workflow YAML parse; `git diff --check` | Next Phase 7 task: document supported deployment topology and upgrade compatibility |
| 2026-06-20 | Phase 7 container finding remediation | Trivy confirmed the Debian runtime packages were clean and attributed all 15 high/critical findings to the Go standard library embedded in PostgreSQL's inherited `gosu` helper. The Collab image overrides that entrypoint and already drops to its dedicated UID directly, so the final runtime now removes unused `gosu` and `docker-entrypoint.sh` while retaining PostgreSQL client tooling required by backup/restore commands. | Docker image build; Trivy `0.71.2` HIGH/CRITICAL OS/library rescan; `docker compose config`; `git diff --check` | Next Phase 7 task: document supported deployment topology and upgrade compatibility |

...
| 2026-06-20 | Phase 7 security-scan follow-up | Cleared the high-severity JavaScript audit findings by updating Vite, React Router, jsdom, and the shadcn CLI dependency graph, while moving shadcn to development scope. Regenerated the lockfile without unsafe transitive overrides; the full audit now selects patched Undici, path-to-regexp, fast-uri, and Hono releases and remains the blocking gate. Changed the container scan to consume a source-built Docker archive directly, corrected the Trivy action tag to `v0.28.0`, verifies the archive exists, and treats unavailable GitHub SARIF/code-scanning ingestion as optional without weakening Trivy's blocking result. | `pnpm audit --audit-level high` (0 high/critical; 6 low, 16 moderate); `pnpm install --frozen-lockfile`; `pnpm test` (517); `pnpm admin:test` (39); `pnpm exec tsc --noEmit`; `pnpm admin:build`; workflow YAML parse; `git diff --check` | Next Phase 7 task: document supported deployment topology and upgrade compatibility |
